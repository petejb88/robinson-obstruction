...
\begin{lemma}[{\cite[\S 4.9]{Rob18}}]
        ...
        \[
                n \mapsto V^{\greenstar}(V^{\wedge n}) %= \pi_{\greenstar} \End(V)
        \]
\end{lemma}
...
\begin{remark}
        ...
        \begin{equation}
                ...
                \qquad \qquad
                V^{\greenstar}(V^{\wedge (-)}) \equiv %\pi_{\greenstar} \mathsf{End}(V) \equiv
...
\begin{corollary}
        ...
        we have an isomorphism between $\Gamma$-homology of $V_{\**}V$ and the cohomotopy of $V^{\**}(V^{\wedge (-)})$. %$\pi_{\**} \mathsf{End}(V)$.
\end{corollary}
\begin{proof}
...
\begin{proof}
        ...
        \[
                \nabla^{n+1}/(\nabla^n \cup \partial \nabla^{n+1})
                \simeq
                E \Sigma_m^{n-m+1}/E \Sigma_m^{n-m} \wedge \tilde T_m/\partial \tilde T_m
                \simeq
                \bigvee_{A} S^{n-m+1} \wedge \bigvee_{(m-1)!} S^{m-2}
                \simeq
                S^{n-1} \wedge (A \times (m-1)!)_+
        \]
        where {\color{red} $A = \Sigma_m^{n-m+2}$}
        and $\mathbb Z[(m-1)!] = \Lie_m^{\**}$.
        Thus, for each $1 \leq m \leq n+1$, we have an obstruction in
        \[
                V^1 \left( \nabla^{n+1}/(\nabla^n \cup \partial \nabla^{n+1}) \wedge_{\Sigma_m} V^{\wedge m} \right)
                =
                V^{2-n}\left( (A \times (m-1)!)_+ \wedge_{\Sigma_m} V^{\wedge m} \right)
        \]
        which by the universal coefficient isomorphism is given by
        \[
        ...
        \todo[inline]{so this is an issue if in fact $A = \Conf_{n-m+2}(\Sigma_m)$ instead and so the number of $\Sigma_m$-s is wrong, as we expect}
